defmodule SymphonyElixir.Codex.TaskAccountabilityRegistry do
  ...
  @profiles %{
    "execution_production" => %{permissions: ["write_bound_scope"], prohibited: ["approve_own_work", "land_without_authority"], independence: ["must_not_self_approve"], closeout: "production"},
    ...
    "response_recovery" => %{permissions: ["write_recovery_scope"], prohibited: ["erase_evidence", "redefine_acceptance"], independence: [], closeout: "recovery"},
  ...
  def compile(attrs) when is_map(attrs) do
    ...
    with :ok <- version_guard(version),
         ...
         :ok <- evidence_guard(evidence, profile, enforced),
  ...
  defp evidence_guard(evidence, profile, true) when is_list(evidence) do
    required = if profile.permissions == ["write_bound_scope"], do: ["change_record", "test_or_quality_evidence"], else: []
    if Enum.all?(required, &(&1 in evidence)), do: :ok, else: {:error, :missing_required_evidence}
  end
