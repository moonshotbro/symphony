defmodule SymphonyElixir.Codex.CoordinationEffects do
  ...
  def dispatch(ledger, intent, :fork, provider) when is_map(provider) do
    with :ok <- validate_fork_intent(intent),
      ...
      CoordinationAdapter.dispatch(
        ledger,
        intent,
        fn -> execute_fork(fork, target_id(intent)) end,
  ...
  defp execute_fork(fork, source_id) do
    case fork.(source_id) do
      {:ok, %{id: child_id, forked_from_id: ^source_id} = child} when is_binary(child_id) and child_id != source_id ->
        {:ok, child, effect(source_id, child_id, Map.get(child, :correlation_id))}

      {:duplicate, %{id: child_id, forked_from_id: ^source_id} = child} when is_binary(child_id) ->
        {:already_satisfied, effect(source_id, child_id, Map.get(child, :correlation_id))}
  ...
  defp inspect_recovered(%Action{kind: :fork, observed_effect: effect} = action, inspect_fork) do
    case Map.get(effect, "fork_thread_id") do
      child_id when is_binary(child_id) ->
        inspect_child(inspect_fork, child_id, Map.get(action.target, "id"))
  ...
  defp inspect_child(inspect_fork, child_id, expected_source) do
    case inspect_fork.(child_id) do
      {:ok, %{id: ^child_id, forked_from_id: ^expected_source}} ->
        {:ok, %{provider: "codex", authoritative: true, exists: true, disposition: "thread_forked"}}
  ...
  defp effect(source_id, child_id, correlation_id) do
    %{"thread_id" => source_id, "fork_thread_id" => child_id, "disposition" => "thread_forked"}
    |> maybe_put("session_correlation_id", correlation_id)
  end
