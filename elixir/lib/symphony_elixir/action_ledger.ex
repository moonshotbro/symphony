defmodule SymphonyElixir.ActionLedger do
  ...
  @source_keys ~w(goal_id task_id issue_id issue_identifier repository revision native_project_id session_id correlation_id fence attempt contract_id title workspace_path worker_host review_source reviewer review_fingerprint review_checks programme role task exact_revision idempotency)
  @target_keys ~w(type id host project worker_host)
  @effect_keys ~w(thread_id turn_id automation_id fork_thread_id destination_thread_id worktree_id revision session_id session_correlation_id worker_host host_assertion disposition workspace_key message_id correlation_id causation_id lifecycle_state sequence)
