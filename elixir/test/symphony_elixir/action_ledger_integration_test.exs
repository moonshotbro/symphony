defmodule SymphonyElixir.ActionLedgerIntegrationTest do
  ...
  test "legacy compensation releases one fresh dispatch and remains exact once across restart" do
    suffix = System.unique_integer([:positive])
    root = Path.join(System.tmp_dir!(), "symphony-legacy-compensation-integration-#{suffix}")
    workspace_root = Path.join(root, "workspaces")
    ledger_path = Path.join(root, "actions.jsonl")
    first_ledger = Module.concat(__MODULE__, "LegacyFirstLedger#{suffix}")
    recovered_ledger = Module.concat(__MODULE__, "LegacyRecoveredLedger#{suffix}")
    final_ledger = Module.concat(__MODULE__, "LegacyFinalLedger#{suffix}")
    orchestrator_name = Module.concat(__MODULE__, "LegacyOrchestrator#{suffix}")
    final_orchestrator_name = Module.concat(__MODULE__, "LegacyFinalOrchestrator#{suffix}")
    issue_id = "issue-legacy-#{suffix}"
    workspace_key = "LEGACY-#{suffix}"
    workspace = Path.join(workspace_root, workspace_key)
    test_pid = self()
    ...
    legacy_intent = %{
      kind: :task_creation,
      source: %{issue_id: issue_id, issue_identifier: workspace_key, revision: "legacy-revision"},
    ...
    assert {:ok, legacy_action, :new} = ActionLedger.plan(first_ledger, legacy_intent)

    assert {:ok, _} =
             ActionLedger.transition(first_ledger, legacy_action.id, :dispatched, %{
               "workspace_key" => workspace_key,
               "disposition" => "worker_spawned"
             })

    GenServer.stop(first_pid)
    {:ok, _} = ActionLedger.start_link(name: recovered_ledger, path: ledger_path, enabled: true)
      ...
      RecoveryInspector.inspect(action,
        thread_lister: fn actual_workspace, worker_host: nil ->
          {:ok,
           [
             %{
               "id" => "thread-legacy-#{suffix}",
               "cwd" => actual_workspace,
               "status" => %{"type" => "notLoaded"},
               "usage" => %{"totalTokens" => 0},
               "turns" => [
                 %{
                   "id" => "turn-legacy-#{suffix}",
                   "status" => "interrupted",
                   "itemsView" => "full",
                   "items" => [],
                   "usage" => %{"totalTokens" => 0}
    ...
    recovered_state = :sys.get_state(orchestrator_name)
    refute MapSet.member?(recovered_state.claimed, issue_id)
    assert {:ok, %{state: :compensated}} = ActionLedger.get(recovered_ledger, legacy_action.id)

    issue = %Issue{
      id: issue_id,
      identifier: workspace_key,
      title: "Fresh issue revision",
      state: "Todo",
      url: "https://github.example/sysmiq/repo/issues/#{suffix}",
      dispatchable: true,
      updated_at: ~U[2026-09-01 04:00:00Z]
    }

    assert Orchestrator.should_dispatch_issue_for_test(issue, recovered_state)

    fresh_intent = %{
      legacy_intent
      | source: %{issue_id: issue_id, issue_identifier: workspace_key, revision: "fresh-revision"},
        checkpoint: "fresh-revision"
    }

    effect = fn ->
      send(test_pid, :fresh_effect_invoked)
      {:ok, :fresh_worker, %{"workspace_key" => workspace_key, "disposition" => "worker_spawned"}}
    end

    assert {:ok, :fresh_worker, fresh_action} =
             CoordinationAdapter.dispatch(recovered_ledger, fresh_intent, effect)

    assert_receive :fresh_effect_invoked
    assert fresh_action.state == :succeeded

    assert {:already_satisfied, %{id: fresh_action_id}} =
             CoordinationAdapter.dispatch(recovered_ledger, fresh_intent, effect)

    assert fresh_action_id == fresh_action.id
    refute_receive :fresh_effect_invoked, 50

    GenServer.stop(orchestrator_name)
    GenServer.stop(recovered_ledger)
    {:ok, _} = ActionLedger.start_link(name: final_ledger, path: ledger_path, enabled: true)

    {:ok, _} =
      Orchestrator.start_link(
        name: final_orchestrator_name,
        action_ledger: final_ledger,
        action_inspector: fn _action -> flunk("terminal actions must not be inspected again") end
      )

    assert {:ok, %{state: :compensated}} = ActionLedger.get(final_ledger, legacy_action.id)
    assert {:ok, %{state: :succeeded}} = ActionLedger.get(final_ledger, fresh_action.id)

    assert {:already_satisfied, %{id: fresh_action_id}} =
             CoordinationAdapter.dispatch(final_ledger, fresh_intent, effect)
